# Deploying the Client Onboarding scenario an Enterprise Pattern Deployment Environment (for CP4BA 22.0.2)

## Introduction

The simplest way to deploy the out-of-box end-to-end [Client Onboarding solution](https://github.com/IBM/cp4ba-client-onboarding-scenario) and its accompanying [labs](https://github.com/IBM/cp4ba-client-onboarding-scenario/blob/main/JamInABox.md#table-of-content) is to use a self-provisioned [Jam-in-a-Box](https://github.com/IBM/cp4ba-client-onboarding-scenario/blob/main/22.0.2/DeployingClientOnboarding2202.md) environment (based on Cloud Pak for Business Automation (CP4BA) 22.0.2 Starter Deployment).

Deploying to a Cloud Pak for Business Automation (CP4BA) 22.0.2 **Enterprise** Deployment environment is also supported. The minimum number of capabilities are: Business Applications, Automation Decision Services, Workflow, Business  Automation Insights, and optionally Automation Document Processing.

This document outlines the different types and approaches to deploy the Client Onboarding scenario that depend how you set up your environment.


## Overview

1. Enterprise Deployment using the [Rapid Deployment scripts](https://github.com/IBM/cp4ba-rapid-deployment) (not yet available for CP4BA 22.0.2) or at least using the [LDIF](https://github.com/IBM/cp4ba-rapid-deployment/blob/main/cp4ba-21-0-3/scripts/predefined.ldif)
   As the users and groups are known several in this configuration fewer 
2. Enterprise Deployment using the [Cloud Pak Deployer](https://ibm.github.io/cloud-pak-deployer/) (specifically for [CP4BA](https://ibm.github.io/cloud-pak-deployer/30-reference/configuration/cp4ba/)) or previously [Apollo one-shot deployment](https://github.com/apollo-business-automation/ibm-cp4ba-enterprise-deployment/)
3. Any other kind of CP4BA Enterprise Deployment environments

Orthogonal to the option above your environment can have [Gitea](https://gitea.io/en-us/) co-deployed in the same cluster or not.

## Prerequisites

1. **Machine to start deployment from**
