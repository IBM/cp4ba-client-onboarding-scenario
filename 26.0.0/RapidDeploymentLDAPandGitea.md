# Deploying the Client Onboarding scenario on an Enterprise Deployment with Rapid Deployment LDAP and co-deployed Gitea (for CP4BA 26.0.0)

## Introduction

These instructions apply in case you have set up your CP4BA 26.0.0 Enterprise deployment 

- using an LDAP with with the users and groups as used in the [Rapid Deployment Scripts](https://github.com/IBM/cp4ba-rapid-deployment/blob/main/cp4ba-21-0-3/03createVMForLDAP.md) (see predefined.ldif & cp4ba.ldif)
- co-deployed [Gitea](https://gitea.io/en-us/) into the same OCP cluster (not necessarily same namespace/project) as CP4BA
...
### Download Required bat/sh File
   ...
   2. **Save the deployment file** that corresponds to the operating system of your deployment machine into the directory created in step 1 (in the context menu of your browser select **Save as.../Save page as...** or similar)
      ...
      **Linux/Mac** - *CP4BA 26.0.0* - Enterprise deployment - **[deploy](https://raw.githubusercontent.com/IBM/cp4ba-client-onboarding-scenario/main/26.0.0/Deployment_Automation/deployClientOnboardingRapidDeploymentEnterpriseWithGitea.sh)** (*Ensure to make the sh file executable by performing `chmod +x deployClientOnboardingRapidDeploymentEnterpriseWithGitea.sh`*)

      **Windows** - *CP4BA 26.0.0* - Enterprise deployment - **[deploy](https://raw.githubusercontent.com/IBM/cp4ba-client-onboarding-scenario/main/26.0.0/Deployment_Automation/deployClientOnboardingRapidDeploymentEnterpriseWithGitea.bat)**

### Update bat/sh File
On Windows open the file **deployClientOnboardingRapidDeploymentEnterpriseWithGitea.bat**/on Linux or Mac open the file **deployClientOnboardingRapidDeploymentEnterpriseWithGitea.sh** in the text editor of your choice. 
...
3. Update the variable `ocpStorageClassForInternalMailServer` with the name of a storage class in your OCP cluster that can be used for the email server and client that will be deployed by default
...
```
Overall execution completed after 24 mins, 42 secs
...
- IBM Content Cortex ACCE: https://<route-prefix>.<clustername>/cpe/acce/
- IBM Content Cortex Admin Desktop: https://<route-prefix>.<clustername>/icn/navigator/?desktop=admin
- IBM Content Cortex GraphQL: https://<route-prefix>.<clustername>/content-services-graphql/
- ICN (Content/ICN lab) Desktop: https://<route-prefix>.<clustername>/icn/navigator/?desktop=ICN
- IBM Workplace (watsonX Orchestrate lab): https://<route-prefix>.<clustername>/icn/navigator/?desktop=workplace
...
- WatsonX Orchestrate: 
  CP4BA-side configuration performed, requires additional separate WatsonX Orchestrate SaaS instance
  MCP Server Target URL: https://<route-prefix>.<clustername>/bas
...
- deployClientOnboarding_26.0.0_Enterprise_output.txt - Contains the messages printed to the console
- deployClientOnboarding_26.0.0_Enterprise_detailedOutput.txt - Everything printed to the console plus details about the deployment steps
- deployClientOnboarding_26.0.0_Enterprise_trace.txt - Contains very detailed trace messages about everything that is done as part of running a deployment
- deployClientOnboarding_26.0.0_Enterprise_combined.txt - Contains a combination of the detailedOutput and trace files
...
Provision an instance of [**IBM Business Automation - Traditional and On-premises. V4.6 - US East Only** ](https://techzone.ibm.com/collection/ibm-business-automation-traditional-and-on-premise/environments). Wait until you receive an email from IBM TechZone with the title "Your environment is ready". 
...
| enableWFGenAI                   | true/false/auto (default: false)                  | Should the genAI activity within Workflow be used as part of the scenario; "auto" will check if it is available in the environment and in case will use it |
| enableWorkplaceAssistant        | true/false/auto (default: false)                  | Should the Workplace Assistant be used as part of the scenario; "auto" will check if it is available in the environment and in case will use it |
...
1. Clone the git repository or alternatively download the [Enterprise](https://github.com/IBM/cp4ba-client-onboarding-scenario/tree/main/26.0.0/Deployment_Automation/Enterprise) folder, [Solution Exports](https://github.com/IBM/cp4ba-client-onboarding-scenario/tree/main/26.0.0/Solution%20Exports) folder, and the [«date»_DeploymentAutomation.jar](https://github.com/IBM/cp4ba-client-onboarding-scenario/tree/main/Deployment_Automation/Current) file
