# Troubleshooting deploying the Client Onboarding scenario into a Enterprise Deployment created by Cloud Pak Deployer - Using an OCP Job (for CP4BA 24.0.0) 

## Introduction

> [!IMPORTANT]
>
> These instructions only apply in case you want to troubleshoot the [OCP Job-based deployment](CloudPakDeployerViaJob.md) of the Client Onboarding scenario to a CP4BA 24.0.0 Enterprise deployment deployed by [**Cloud Pak Deployer**](CloudPakDeployerViaJob.md).
>
...
- deployClientOnboarding_24.0.0_Enterprise_output.txt - Contains the messages printed to the console
- deployClientOnboarding_24.0.0_Enterprise_detailedOutput.txt - Everything printed to the console plus details about the deployment steps
- deployClientOnboarding_24.0.0_Enterprise_trace.txt - Contains very detailed trace messages about everything that is done as part of running a deployment
- deployClientOnboarding_24.0.0_Enterprise_combined.txt - Contains a combination of the detailedOutput and trace files
...
4. **Enter** `ls -l` to list the files in the directory. Using `vi deployClientOnboarding_24.0.0_Enterprise_output|detailedOutput|trace|combined.txt`, you can look at content of the different log files
...
6. Type `oc cp icp4adeploy-bastudio-deployment-0:/logs/application/client-onboarding/client-onboarding-deploy-<id of the pod>/deployClientOnboarding_24.0.0_Enterprise_output|detailedOutput|trace|combined.txt ./deployClientOnboarding_24.0.0_Enterprise_output|detailedOutput|trace|combined.txt` to copy the respective file to your local machine.
